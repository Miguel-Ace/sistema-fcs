...
@section('contenido')
    ...
    <div class="container">
        ...
        @role('admin')
        <a href="{{url('/asistencias/create')}}" class="btn btn-warning mb-3">
                ...
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Sigla Asistencia</th>
                            @role('admin')
                    ...
                    <tbody>
                        @foreach ($datos as $dato)
                            <tr class="text-center">
                                ...
                                <td>{{$dato->fecha}}</td>
                                <td>{{$dato->sigla_asistencia}}</td>
                                @role('admin')
                                <td>
                                    <a href="{{url('asistencias/'.$dato->id)}}" class="btn btn-primary"><ion-icon name="eye-outline"></ion-icon></a>
                                    |
                                    <a href="{{url('asistencias/'.$dato->id.'/edit')}}" class="btn btn-success"><ion-icon name="brush-outline"></ion-icon></a>

                                    {{-- <form action="{{'asistencias/'.$dato->id}}" method="post" class="d-inline">
                                        @csrf
                                        {{method_field('DELETE')}}
